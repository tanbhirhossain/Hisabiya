...
import { ref } from 'vue';
import { Plus, Trash2, PiggyBank, X } from 'lucide-vue-next';

const props = defineProps<{
    goals: Array<{ id: number; name: string; target_amount: number; current_amount: number; deadline: string | null; color?: string; icon?: string; status: string; progress_percent: number }>;
}>();

const addOpen = ref(false);
const addForm = useForm({ name: '', target_amount: '', deadline: '', color: '#10b981' });
